...
struct LimitStatusView: View {
    ...
    var statusText: String {
        ...
        if diff == 0 {
            return String(localized: "limit_exact")
        } else if diff > 0 {
            if diff == 1 {
                return String(localized: "limit_slightly_over")
            } else {
                return String(localized: "limit_over")
            }
        } else {
            if diff == -1 {
                return String(localized: "limit_slightly_under")
            } else {
                return String(localized: "under_limit_emoji")
            }
        }
    }
...
#Preview {
    ContentView()
        .environmentObject(DataStore())
            // Wymusza renderowanie w uproszczonej przestrzeni barw sRGB (duża ulga dla grafiki Iris)
        .colorScheme(.light)
            // Wyłącza animacje systemowe podczas ładowania podglądu
}
